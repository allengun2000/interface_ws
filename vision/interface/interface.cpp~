...
void InterfaceProc::imageCb(const sensor_msgs::ImageConstPtr& msg){
	...
	*frame = cv_ptr->image;
	...
	if(onchick==1){Omni_distance(mousex-robotCenterX,mousey-robotCenterY);onchick=0;}
...
cv::Mat InterfaceProc::CenterModel(const cv::Mat iframe){
	...
	x=robotCenterX+lengh*cos(FrontMsg*PI/180),    y=robotCenterY+lengh*sin(FrontMsg*PI/180);//check "y" robotCenterY"-"lengh*sin
...
int interface_window::Angle_Interval(int radius){ 
